...
const SortableItem = SortableElement(({ value }) => (
  <div className={imageStyle.layerDiv}>
    ...
    <span
      ...
      data-id={value.id}
      data-name={value.name}
      className={imageStyle.layerDelete}
    >
      x
    </span>
  </div>
    ...
    <ul>
      {items.map((value, index) => (
        <SortableItem
          key={`item-${index}`}
          index={index}
          value={{ id: value.layerid, name: value.name }}
        />
      ))}
    </ul>
  );
...
class SortableComponent extends Component {
  ...
  onSortEnd = async ({ oldIndex, newIndex }) => {
    ...
    this.layerNamesArray = arrayMoveImmutable(
      this.layerNamesArray,
      oldIndex,
      newIndex
    );
    this.layerIdArray = arrayMoveImmutable(
      this.layerIdArray,
      oldIndex,
      newIndex
    );
    ...
    this.setState(
      ...
      async () => {
        ...
        for (let i = 0; i < Object.keys(this.layerIdArray).length; i++) {
          var layerIndex = this.layerIdArray
            .map(function (x) {
              return x;
            })
            .indexOf(this.layerIdArray[i]);
          await db.layerNames.update(parseInt(this.layerIdArray[i]), {
    ...
    return (
      <div>
        ...
        <Modal show={this.state.showConfirmation} onHide={this.handleClose}>
          ...
          <Modal.Body>
            Removing this layer will remove all images in this layer.
          </Modal.Body>
          <Modal.Footer>
